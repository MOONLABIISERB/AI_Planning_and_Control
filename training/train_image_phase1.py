...
try:
    ...
    sys.path.append(glob.glob('../PythonAPI/agents')[0])
...
from bird_view.utils.train_utils import one_hot
from bird_view.utils.datasets.image_lmdb import get_image as load_data
...
PIXELS_PER_METER = 5
CROP_SIZE = 192
SAVE_EPOCHS = np.arange(0, 1000, 4)
...
def _log_visuals(rgb_image_left, rgb_image_right, birdview, speed, command, loss, pred_locations, _pred_locations, _teac_locations, size=8):
    import cv2
    import numpy as np
    ...
    for i in range(min(birdview.shape[0],size)):
        ...
        rgb_left = np.uint8(_numpy(rgb_image_left[i]).transpose(1, 2, 0) * 255).copy()
        rgb_right = np.uint8(_numpy(rgb_image_right[i]).transpose(1, 2, 0) * 255).copy()
        ...
        def _stick_together(a, c,  b):
            ...
            c = cv2.resize(b, (int(r3 * c.shape[1]), int(r3 * c.shape[0])))
    
            return np.concatenate([a, c, b], 1)
        ...
        for x, y in _pred_locations[i]: _dot(rgb_left, x, y, RED)
        for x, y in _pred_locations[i]: _dot(rgb_right, x, y, RED)
        ...
        images.append((loss[i].item(), _stick_together(rgb_left, rgb_right, canvas)))
...
def train_or_eval(coord_converter, criterion, net, teacher_net, data, optim, is_train, config, is_first_epoch):
    ...
    # noiser = tdist.Normal(torch.tensor(0.0), torch.tensor(0.0))
    loss_ = []
    for i, (rgb_image_left, rgb_image_right, birdview, location, command, speed) in iterator:
        # y_l = rgb_image_left[0].cpu().detach().numpy()
        # y_r = rgb_image_right[0].cpu().detach().numpy()

        # plt.imshow(np.transpose(np.concatenate((y_l, y_r), axis=1), (1, 2, 0)))
        # plt.show()
        ...
        if len(rgb_image_left.size()) > 4:
            B, batch_aug, c, h, w = rgb_image_left.size()
        ...
        if should_log:
            ...
            # images = _log_visuals(
            #         rgb_image_left, rgb_image_right, birdview, speed, command, loss,
        ...
        #     bzu.log.image(is_train=is_train, birdview=images)
            # for v in images:
            #     writer.add_image('image', _preprocess_image(v), j)
            # j+=1
...
def train(config):
    ...
    net.load_state_dict(torch.load(config['phase0_ckpt'], map_location=torch.device('cuda')))
    total_params = sum(
    ...
    teacher_net.load_state_dict(torch.load(config['teacher_args']['model_path'], map_location=torch.device('cuda')))
    teacher_net.eval()
    ...
    for epoch in tqdm.tqdm(range(897, config['max_epoch']), desc='Epoch'):
        train_loss = train_or_eval(coord_converter, criterion, net, teacher_net, data_train, optim, True, config, epoch == 701)
        val_loss = train_or_eval(coord_converter, criterion, net, teacher_net, data_val, None, False, config, epoch == 701)
        writer.add_scalar('Loss/train', train_loss, epoch)
        writer.add_scalar('Loss/val', val_loss, epoch)
        # writer.add_image('image', _preprocess_image(images), epoch)

        if epoch in SAVE_EPOCHS:
            torch.save(
                    net.state_dict(),
                    str(Path(config['log_dir']) / ('checkpoints') / ('phase1') / ('model-%d.th' % epoch)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', default='/media/storage/karthik/lbc/checkpoints/phase1/model-896.th')
    ...
    parser.add_argument('--dataset_dir', default='/media/storage/karthik/lbc/data')
    parser.add_argument('--batch_size', type=int, default=96)
    parser.add_argument('--speed_noise', type=float, default=0.0)
    parser.add_argument('--augment', choices=['medium', 'medium_harder', 'super_hard', 'None', 'custom'], default='None')
