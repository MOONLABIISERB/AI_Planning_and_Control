...
def train_or_eval(coord_converter, criterion, net, teacher_net, data, optim, is_train, config, is_first_epoch):
    ...
    for i, (rgb_image_left, rgb_image_right, birdview, location, command, speed) in iterator:
        ...
        # writer.add_scalar("loss/train_epoch{}".format(i), loss_mean.item(), i)
        if should_log:
            metrics = dict()
            metrics['loss'] = loss_mean.item()

            images = _log_visuals(
                    rgb_image_left, birdview, speed, command, loss,
...
def train(config):
    ...
    net.load_state_dict(torch.load('/home/phoenix/karthik/ckpts/image_new/model.th'))
    total_params = sum(
    ...
    for epoch in tqdm.tqdm(range(config['max_epoch']+1), desc='Epoch'):
        ...
        if epoch in SAVE_EPOCHS:
            torch.save(
                    net.state_dict(),
                    str(Path(config['log_dir']) / ('checkpoints') / ('phase0') / ('model-%d.th' % epoch)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='/home/phoenix/karthik')
    ...
    # Teacher.
    parser.add_argument('--teacher_path', default='/home/phoenix/karthik/ckpts/priveleged/model-128.th')
    
    parser.add_argument('--fixed_offset', type=float, default=4.0)

    # Dataset.
    parser.add_argument('--dataset_dir', default='/home/phoenix/karthik/dataset')
