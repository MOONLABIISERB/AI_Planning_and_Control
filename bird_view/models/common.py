...
class Normalize(nn.Module):
    ...
    def cuda(self):
        self.mean = self.mean.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        self.std = self.std.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
...
class NormalizeV2(nn.Module):
    def __init__(self, mean, std):
        ...
        self.mean = torch.FloatTensor(mean).reshape(1, 3, 1, 1).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        self.std = torch.FloatTensor(std).reshape(1, 3, 1, 1).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
