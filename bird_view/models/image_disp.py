...
from . import common_new as common
...
def rgb2bgr(image):
    image = np.concatenate((image[:, 2, :, :], image[:, 1, :, :], image[:, 0, :, 1]))
    return image
...
class ImagePolicyModelSS(common.ResnetBase):
    ...
    def forward(self, image_left, image_right, velocity, command):
        ...
        with torch.no_grad():
            image_left = np.transpose(image_left.cpu().clone().numpy(), (0, 3, 2, 1))
            image_right = np.transpose(image_right.cpu().clone().numpy(), (0, 3, 2, 1))
            ...
            for i in range(image_left.shape[0]):
                ...
                img_l.append(cv.cvtColor(image_left[i], cv.COLOR_BGR2GRAY))
                img_r.append(cv.cvtColor(image_right[i], cv.COLOR_BGR2GRAY))
                image_disparity.append(stereo.compute(img_l[i].astype(np.uint8),img_r[i].astype(np.uint8)))
                image.append(np.concatenate((np.expand_dims(img_l[i], axis=-1), np.expand_dims(img_r[i], axis=-1), np.expand_dims(image_disparity[i], axis=-1)), axis=-1))
            image = np.transpose(image, (0, 3, 2, 1))

        image = self.rgb_transform(torch.FloatTensor(image).to('cuda'))
