...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rgb_image_left = np.fromstring(lmdb_txn.get(('rgb_left_%04d'%index).encode()), np.uint8).reshape(160,384,3)
        rgb_image_right = np.fromstring(lmdb_txn.get(('rgb_right_%04d'%index).encode()), np.uint8).reshape(160,384,3)
...
def get_image(
    ...
    def make_dataset(dir_name, is_train):
        ...
        _samples = 703 if is_train else 124
...
if __name__ == '__main__':
    ...
    mean = []
    for rgb_img_left, rgb_img_right, bird_view, locations, cmd, speed in tqdm.tqdm(loader):
        mean.append(rgb_img_left.mean(dim=(0,2,3)).numpy())
